...
<body class="min-h-screen bg-slate-50">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside id="sidebar" class="fixed top-0 left-0 z-50 h-screen w-64 bg-white text-slate-900 transform -translate-x-full transition-transform duration-300 ease-in-out lg:translate-x-0 lg:relative border-r border-slate-200">
            <div class="flex flex-col h-full">
                ...
                <div class="flex items-center justify-between h-20 px-6 border-b border-slate-200">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-emerald-600 rounded-lg flex items-center justify-center">
                            <i class="fas fa-home text-white" data-lucide="home"></i>
                        </div>
                        <div>
                            <h1 class="font-semibold text-base">QR Intercom</h1>
                            <p class="text-xs text-slate-500">Resident Portal</p>
                        </div>
                    </div>
                ...
                <nav class="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
                    <?php foreach ($this->user_nav as $item): ?>
                        <?php 
                ...
                <div class="p-3 border-t border-slate-200">
                    <a href="<?php echo BASE_URL; ?>/logout?type=user" onclick="confirmLogout(event)" class="nav-link flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-slate-600">
        ...
        <div class="flex-1 flex flex-col min-h-screen overflow-hidden">
            ...
            <header class="bg-white border-b border-slate-200 px-6 h-20 flex items-center justify-between flex-shrink-0 shadow-sm">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="relative group">
                        ...
                        <div id="notificationDropdown" class="absolute right-0 top-full mt-2 w-80 bg-white rounded-lg shadow-lg border border-slate-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                            <div class="p-4" id="dropdownContent">
                                <?php if (!empty($unread_notifications)): ?>
                                <div class="flex items-center justify-between mb-3">
                                    <h3 class="text-sm font-semibold text-slate-900">Recent Notifications</h3>
                                    <span class="text-xs text-slate-500" id="dropdownCount"><?php echo $unread_count; ?> unread</span>
                                </div>
                                <div class="space-y-2 max-h-64 overflow-y-auto" id="dropdownNotifications">
                                    <?php 
                                    $display_count = 0;
                    ...
                    <div class="flex items-center gap-3 pl-4 border-l border-slate-200">
                        <div class="w-10 h-10 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center text-white font-semibold">
                            <?php echo strtoupper(substr($resident_info['name'] ?? 'U', 0, 1)); ?>
                        </div>
                        <div class="hidden sm:block">
                            <p class="text-sm font-medium text-slate-900"><?php echo htmlspecialchars($resident_info['name'] ?? 'User'); ?></p>
            ...
            <main class="flex-1 overflow-auto p-6">
                ...
                if (strpos($uri, '/user/dashboard') !== false) {
                    require VIEWS_PATH . '/pages/user/dashboard.php';
                } elseif (strpos($uri, '/user/appointments') !== false) {
                    require VIEWS_PATH . '/pages/user/appointments.php';
                } elseif (strpos($uri, '/user/visitor-log') !== false) {
                    require VIEWS_PATH . '/pages/user/visitor_log.php';
                } else {
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
                ...
                <div class="flex items-center justify-between p-6 border-b border-slate-200">
                    <div>
                        ...
                        <p class="text-slate-600 mt-1">Stay updated with your apartment activities</p>
                    </div>
    ...
    <script>
        ...
        function toggleNotificationRead(notificationId) {
            fetch('<?php echo BASE_URL; ?>/user/notifications', {
                method: 'POST',
                headers: {
        ...
        let lastNotificationIds = new Set(<?php echo json_encode(array_column($unread_notifications, 'id')); ?>);
        ...
        function pollNotifications() {
            fetch('<?php echo BASE_URL; ?>/user/notifications', {
                ...
                body: 'action=get_recent_notifications'
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    ...
                    if (currentCount > lastNotificationCount) {
                        ...
                        newNotifications.forEach(notification => {
                            const message = notification.message.length > 80 
                    ...
                    if (currentCount > 0) {
                        if (countBadge) {
                            countBadge.textContent = currentCount > 99 ? '99+' : currentCount;
                        } else {
                            const badge = document.createElement('span');
                            badge.className = 'absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-semibold';
                ...
                if (data.success) {
                    ...
                    if (notifications.length > 0) {
                        ...
                        dropdownContent.innerHTML = `
                            <div class="flex items-center justify-between mb-3">
                                <h3 class="text-sm font-semibold text-slate-900">Recent Notifications</h3>
                                <span class="text-xs text-slate-500">${count} unread</span>
                            </div>
        ...
        // Add hover event listener to update dropdown content
        document.addEventListener('DOMContentLoaded', function() {
            const notificationDropdown = document.getElementById('notificationDropdown');
            if (notificationDropdown) {
                const parentGroup = notificationDropdown.closest('.group');
                if (parentGroup) {
